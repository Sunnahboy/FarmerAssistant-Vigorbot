...
def upload_image():
    if 'image' in request.files:
        ...
        # For demonstration, encode the image as base64
        encoded_image = base64.b64encode(image_file.read()).decode('utf-8')

        # Use Gemini Pro Vision to generate content based on the image
        ...
        # Get the trimmed text from the response
        trimmed_text = response.text[:100] + "... [TRIMMED] ..."

        return jsonify({'image': encoded_image, 'response': trimmed_text})
    else:
